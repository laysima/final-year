...
const ProductPage = ({params}:any) => {
  ...
  const doubleSubtotal = () => {
    ...
    add_to_cart({ id:id, imageUrl: product.imageUrl, name: product.name, description: product.description, price: product.price, quantity: counter, currency: '', subtotal: (product.price * counter) }); 

  };

  const halveSubtotal = () => {
     setCounter(prevCounter => Math.max(1, prevCounter - 1));
     add_to_cart({ id:id, imageUrl: product.imageUrl, name: product.name, description: '', price: product.price, quantity: counter, currency: '', subtotal: (product.price * counter) });
  ...
  return (
    ...
    {/*<Text>we are good: {id}</Text*/}
      ...
      <Flex direction={'column'} maxW={'25%'} gap={4} mt={'50px'}>
          ...
          <Flex alignItems={'center'} _hover={{color:'teal', cursor:'pointer'}}>
